...
class NERSpacyRanker(Match2DocRanker):
    ...
    def post_init(self):
        ...
        # TODO check if isntalled, if not downlaod
    ...
    def _recalc_score_matches(self, matches_metas, old_matches_scores, query_meta):
        ...
        for match_score, match in zip(old_matches_scores, matches_metas):
            ...
            entity_match_score = self._entity_match(match_ents, query_ents)

            new_match_score = self._recalc_match_score(match_score, entity_match_score)
            print(
                f'#### match text = {match["text"].strip()}; orig score = {match_score}; new score = {new_match_score}; match type = {entity_match_score}'
            )
